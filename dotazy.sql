...
SELECT 
    ...
    Films.Release_Year,         -- rok vydání
    Directors.Name AS Director  -- jméno režiséra
FROM Films
INNER JOIN Directors ON Films.Director_ID = Directors.Director_ID; -- spojení podle režiséra

-- 2. Filmy s herci a jejich rolemi
CREATE VIEW film_actor_summary AS
SELECT 
    Films.Title AS Film_Title,  -- název filmu
    Actors.Name AS Actor_Name,  -- jméno herce
    Film_Actors.Role            -- role ve filmu
FROM Film_Actors
INNER JOIN Films ON Film_Actors.Film_ID = Films.Film_ID         -- spojení na film
INNER JOIN Actors ON Film_Actors.Actor_ID = Actors.Actor_ID;   -- spojení na herce
